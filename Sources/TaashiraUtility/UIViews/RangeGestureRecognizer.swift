...
public class RangeGestureRecognizer: UITapGestureRecognizer {
    ...
    func didTapAttributedTextInLabel(
        ...
        let locationOfTouchInLabel = self.location(
            in: label
        )
        let textBoundingBox = layoutManager.usedRect(
            for: textContainer
        )
        let textContainerOffset = CGPoint(
            x: (
                labelSize.width - textBoundingBox.size.width
